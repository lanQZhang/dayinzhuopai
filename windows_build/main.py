...
FONT = 'Microsoft YaHei' if SYSTEM == 'Windows' else 'PingFang SC'
...
class NameCardApp:
    ...
    def _build_ui(self):
        tk.Label(self.root, text='桌牌生成器', font=(FONT, 20, 'bold')).pack(pady=(20, 5))
        tk.Label(self.root, text='将Excel文件拖入下方区域，或点击选择文件', font=(FONT, 10), fg='gray').pack()
        ...
        drop_label = tk.Label(drop_frame, text='拖入Excel文件到这里\n或点击此处选择文件',
                              font=(FONT, 12), fg='#888', bg='#f0f0f0', cursor='hand2')
        ...
        self.file_label = tk.Label(self.root, textvariable=self.excel_path,
                                   font=(FONT, 9), fg='#333', wraplength=420, anchor='w')
        ...
        self.status_label = tk.Label(self.root, text='', font=(FONT, 9), fg='gray')
        self.status_label.pack(pady=2)

        btn_frame = tk.Frame(self.root)
        btn_frame.pack(pady=10)

        self.gen_btn = tk.Button(btn_frame, text='生成桌牌PDF', font=(FONT, 11),
                                 bg='#4a90d9', fg='white', width=14, cursor='hand2',
                                 command=self._generate)
        self.gen_btn.pack(side='left', padx=8)

        self.tpl_btn = tk.Button(btn_frame, text='下载填写模板', font=(FONT, 11),
